...
def format_btc_twd_holding(result: dict) -> str:
    ...
    # 計算美元均價與市價
    usdtwd = result.get("usdtwd") or 32.0
    avg_price_usd = avg_price_twd / usdtwd if usdtwd else 0.0
    btc_usd = result.get("btc_usd")
    if btc_usd is None and usdtwd:
        btc_usd = price_twd / usdtwd
    if btc_usd is None:
        btc_usd = 0.0
    ...
    return "\n".join([
        ...
        f"   買入均價 ${avg_price_usd:,.0f}/BTC",
        f"   市場單價 ${btc_usd:,.0f}/BTC",
